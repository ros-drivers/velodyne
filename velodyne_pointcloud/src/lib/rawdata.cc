    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE)
    {
           ...
           (raw->blocks[i].rotation <= config_.max_angle || raw->blocks[i].rotation >= config_.min_angle)))
      {
        ...
        if (tmp.uint == 0)  // no valid laser beam return
        {
          ...
          data.addPoint(nanf(""), nanf(""), nanf(""), corrections.laser_ring, raw->blocks[i].rotation, nanf(""),
                        nanf(""), time);
          continue;
        }
        ...
        float cos_rot_angle =
            cos_rot_table_[block.rotation] * cos_rot_correction + sin_rot_table_[block.rotation] * sin_rot_correction;
        float sin_rot_angle =
            sin_rot_table_[block.rotation] * cos_rot_correction - cos_rot_table_[block.rotation] * sin_rot_correction;
